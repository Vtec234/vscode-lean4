...
import { InteractiveMessageFormat } from './interactiveExpr/InteractiveExpr'
import { getInteractiveDiagnostics, InteractiveDiagnostic, TaggedText } from './interactiveExpr/rpcInterface'
...
function stripTags<T>(t: TaggedText<T>): string {
...
const MessageView = React.memo(({uri, diag}: MessageViewProps) => {
    ...
    const text = stripTags(diag.message)
    ...
    <details open>
        ...
        <div className="ml1">
            <pre className="font-code" style={{whiteSpace: 'pre-wrap'}}>
                <InteractiveMessageFormat pos={{uri: loc.uri, line: loc.range.start.line, character: loc.range.start.character}} fmt={diag.message} />
            </pre>
        </div>
    </details>
    )
